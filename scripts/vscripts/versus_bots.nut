if(!("g_VersusBotTriggers" in getroottable()))
	getroottable()["g_VersusBotTriggers"] = this;
...
SessionOptions <-
{
	...
	weaponsToConvert =
	{
		...
		weapon_rifle 			= "weapon_rifle_spawn"
		weapon_rifle_desert	 	= "weapon_rifle_desert_spawn"
		weapon_rifle_sg552		= "weapon_rifle_sg552_spawn"
		weapon_rifle_ak47		= "weapon_rifle_ak47_spawn"
		weapon_smg_mp5			= "weapon_smg_mp5_spawn"
		weapon_sniper_military	= "weapon_sniper_military_spawn"
		weapon_sniper_awp		= "weapon_sniper_awp_spawn"
	...
	{
		// bots take only smg
		weapon_upgradepack_explosive = true
		weapon_upgradepack_incendiary = true
		weapon_melee = true
	}
	defaultItems =
	[
...
function force_set_cvar()
{
	...
	Convars.SetValue("sb_combat_saccade_speed", 800);
	Convars.SetValue("sb_normal_saccade_speed", 800);
	...
	Convars.SetValue("versus_tank_chance_intro", 0.75);
	Convars.SetValue("versus_tank_chance_finale", 0.1);
	Convars.SetValue("versus_tank_chance", 0.3);
	...
	Convars.SetValue("z_witch_damage_per_kill_hit", 100);
	Convars.SetValue("z_witch_personal_space", 100);
	Convars.SetValue("z_leap_interval_post_incap", 15);
	Convars.SetValue("z_jockey_control_variance", 0.15);
	Convars.SetValue("z_tank_attack_interval", 1.5);
...
function Notifications::OnRoundBegin::VersusBots_RoundStart(params)
{
	...
	g_VersusBotTriggers.g_is_versus_mode = (Director.GetGameMode() in ["versus", "scavenge", "mutation11", "mutation12", "mutation13", "mutation15", "mutation18"]);
	printl("g_is_versus_mode = " + g_VersusBotTriggers.g_is_versus_mode);
}
...
function Notifications::OnFirstSpawn::VersusBots_GetMapName(client, params)
{
	...
	if (player && player.IsPlayer() && IsPlayerABot(player) && !player.IsSurvivor() && player.GetZombieType() != Z_TANK) {
		...
		g_VersusBotTriggers.g_si_pos_check_time[params.userid] <- Time();
	}
}
...
function Notifications::OnHurt::VersusBots_BotAttack(victim, attack, params)
{
	local player = GetPlayerFromUserID(params.userid);
	if (("attackerentid" in params) && player && player.IsPlayer() && player.IsSurvivor()) {
		...
		g_VersusBotTriggers.g_bot_hurt_time = Time();
		if (attacker && attacker.IsPlayer() && !attacker.IsSurvivor() && attacker.GetZombieType() == Z_TANK) {
			g_VersusBotTriggers.g_bot_hurt_tank_time = Time();
		} else if (!(attacker && attacker.IsPlayer() && attacker.IsSurvivor())) {
			g_VersusBotTriggers.g_bot_hurt_without_tank_time = Time();
		}
	}
...
DMG_BOT_TANK <- 24.0
DMG_BOT_TANK_CAR <- 100.0
// function AllowTakeDamage( damageTable )
function EasyLogic::OnTakeDamage::VersusBots_DamageHook(damageTable)
{
	...
	if (victim && victim.IsPlayer() && victim.IsSurvivor() && victim.IsBot()) {
		// survivor bot
		if (damageTable.DamageDone > 1.0) {
			if (attacker && attacker.IsPlayer() && !attacker.IsSurvivor() && attacker.GetType() == Z_TANK) {
				// tank
				if (damageTable.DamageDone >= 99.0) {
					// car
					damageTable.DamageDone = g_VersusBotTriggers.DMG_BOT_TANK_CAR;
				} else {
					damageTable.DamageDone = g_VersusBotTriggers.DMG_BOT_TANK;
				}
			} else if (g_VersusBotTriggers.g_all_bot_survivor_team && attacker && attacker.IsPlayer() && !attacker.IsSurvivor()) {
				// all bot team
				damageTable.DamageDone = damageTable.DamageDone * 1;
			}
			if (damageTable.DamageDone < 1.0) {
				damageTable.DamageDone = 1.0;
			}
			if (g_VersusBotTriggers.g_bot_move_target) {
				damageTable.DamageDone = 1.0;
			}
		}
	}
...
function get_remote_pills()
{
	...
	if (health_avg < 45.0) {
		local pills = null;
		while (pills = Entities.FindByClassname(pills, "weapon_pistol_spawn")) {
			if (bots.len() > i) {
				DoEntFire("!self", "Use", "", 0.0, bots[i++], pills);
				// Say(null, "bot got a remote pistol", false);
			}
		}
	}
}
...
function update_survivor_bot_ai()
{
	...
	if (has_tank()) {
		...
		while (tank = Entities.FindByClassname(tank, "player")) {
			if (!tank.IsSurvivor() && !tank.IsDead() && tank.GetZombieType() == Z_TANK) {
			...
			if (player && player.IsPlayer() && IsPlayerABot(player) && !player.IsSurvivor() && player.GetZombieType() != Z_TANK) {
				if (t - g_si_pos_check_time[id] > STUCKED_DETECT_TIME) {
					...
					if (diff < STUCK_RADIUS) {
						// Say(player, "Killed stucked bot", false);
						player.Kill();
					} else {
						g_si_pos[id] <- pos;
					}
...
function respawn_check()
{
	...
	foreach (si in sis) {
		if (IsPlayerABot(si) && si.GetZombieType() != Z_TANK) {
			...
			if (near_dist > SI_RESPAWN_DIST && svs.len() > 0 && GetFlowDistanceForPosition(svs[0].GetOrigin()) > GetFlowDistanceForPosition(si.GetOrigin())) {
				...
				if (!visible) {
					...
					SessionOptions.cm_SpecialRespawnInterval <- new_interval;
					SessionOptions.SpecialRespawnInterval <- new_interval;
					si.Kill();
				}
			}
		}
...
function si_reset()
{
	...
	g_si_spawn_target_time = 0;
	g_si_first_spawn_time = 0;
	SessionOptions.cm_SpecialRespawnInterval <- interval;
...
function si_bot_spawned(si)
{
	...
	{
		SessionOptions.PreferredSpecialDirection <- SI_SPAWN_DIR[g_si_count % SI_SPAWN_DIR.len()];
	}
...
// function Update()
function EasyLogic::Update::VersusBots_StateUpdate()
{
	local t = Time();
	...
	if (!g_VersusBotTriggers.g_bot_start) {
		if (t > g_VersusBotTriggers.g_bot_start_time) {
			...
			g_VersusBotTriggers.g_bot_start = true;
	...
	if (t > g_VersusBotTriggers.g_all_bot_check_time) {
		if (g_VersusBotTriggers.all_bot_team()) {
			...
			if(g_VersusBotTriggers.g_is_versus_mode)
			{
				...
				// SessionOptions.ZombieTankHealth <- 5000;
			}
		} else {
		...
		g_VersusBotTriggers.g_all_bot_check_time = Time() + 10.0;
	}
	...
	if (!g_VersusBotTriggers.g_set_spawn_range) {
		if (!("ZombieSpawnRange" in SessionOptions)) {
			SessionOptions.ZombieSpawnRange <- Convars.GetFloat("z_spawn_range");
		}
		g_VersusBotTriggers.g_set_spawn_range = true;
	}
	if (g_VersusBotTriggers.g_si_spawn_target_time - 1.0 < t) {
		if (g_VersusBotTriggers.g_zombie_spawn_range == 0) {
			g_VersusBotTriggers.debug_print("set spawn range");
			g_VersusBotTriggers.g_zombie_spawn_range = SessionOptions.ZombieSpawnRange;
			SessionOptions.ZombieSpawnRange <- g_VersusBotTriggers.SI_SPAWN_RANGE;
			g_VersusBotTriggers.g_change_spawn_range_time = t;
		}
	} else if (g_VersusBotTriggers.g_zombie_spawn_range > 0 && t > g_VersusBotTriggers.g_change_spawn_range_time + 3.0) {
		if (SessionOptions.ZombieSpawnRange == g_VersusBotTriggers.SI_SPAWN_RANGE) {
			g_VersusBotTriggers.debug_print("reset spawn range");
			SessionOptions.ZombieSpawnRange <- g_VersusBotTriggers.g_zombie_spawn_range;
		}
